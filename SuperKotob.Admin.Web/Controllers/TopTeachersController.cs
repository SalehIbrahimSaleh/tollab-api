...
using Tollab.Admin.UseCases.TopTeachers;

namespace Tollab.Admin.Web.Controllers
{
    [Authorize]
    public class TopTeachersController : BaseWebController<TopTeachers, TopTeachers>
    {
        TopTeachersService _topTeachersService;
        public TopTeachersController(TopTeachersService service, IAppConfigurations appConfigurations) : base(service, appConfigurations)
        {
            _topTeachersService = service;
        }
        ...
        public  async Task<ActionResult> ShowTop(long? CountryId)
        {
            try
            {
                ...
                var response = await _topTeachersService.GetAsync(requestInputs);
